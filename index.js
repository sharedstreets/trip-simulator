    ...
    if (
      ...
      osrm.route({
        ...
        if(!err && result.routes[0]) {
          var overview = turf.lineString(result.routes[0].geometry.coordinates)
          ...
          if (distance > 0) {
            ...
            while (playback <= duration) {
              ...
              timestamps.push(record.timeA.add(playback, 'seconds').format())
            }

            if(trip.length >= 2) {
              console.log(JSON.stringify(turf.lineString(trip, {timestamps: timestamps})))
            }
          }
        }
        next()
